...
inline __device__ void warpBitonicMergeLE16(K& k, V& v) {
    ...
    if (!IsBitonic) {
        ...
        bool small_var = !(laneId & L);

        if (Dir) {
            // See the comment above how performing both of these
            // comparisons in the warp seems to win out over the
            // alternatives in practice
            bool s = small_var ? Comp::gt(k, otherK) : Comp::lt(k, otherK);
            assign(s, k, otherK);
            assign(s, v, otherV);

        } else {
            bool s = small_var ? Comp::lt(k, otherK) : Comp::gt(k, otherK);
            assign(s, k, otherK);
            assign(s, v, otherV);
        }
    }
